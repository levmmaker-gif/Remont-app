...
const INIT_PRICELISTS = [
  { id: 1, name: "Отделочные работы", items: [{ id: 1, name: "Штукатурка стен", unit: "м²", price: 650 }, { id: 2, name: "Шпаклёвка стен", unit: "м²", price: 450 }, { id: 3, name: "Покраска стен", unit: "м²", price: 300 }] },
  { id: 2, name: "Напольные работы", items: [{ id: 1, name: "Укладка плитки", unit: "м²", price: 1200 }, { id: 2, name: "Укладка ламината", unit: "м²", price: 600 }] },
];
...
const INIT_PROJECTS = [
  ...
  },
  {
    ...
    costItems: [{ id: 1, name: "Сантехника", executor: "СантехМастер", price: 200000, expenses: 180000, works: "Разводка труб", done: true, progress: 100, payments: [{ id: 1, date: "2025-07-01", amount: 100000, note: "Аванс" }, { id: 2, date: "2025-07-20", amount: 100000, note: "Финал" }] }],
    participants: [],
    photos: [],
    agentFees: [],
  },
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .bp{background:#E8643A;color:#fff;border:none;border-radius:13px;padding:15px;font-size:16px;font-weight:600;font-family:inherit;width:100%;cursor:pointer;margin-top:8px}
  ...
  .ptab{white-space:nowrap;padding:7px 14px;border-radius:20px;font-size:13px;font-weight:500;cursor:pointer;background:#fff;color:#888;border:1.5px solid #E5E3DE;flex-shrink:0}
  ...
  .fab{position:absolute;bottom:92px;right:20px;width:54px;height:54px;background:#E8643A;border-radius:16px;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 6px 20px #E8643A55;z-index:10}
  ...
  .estcheck{width:22px;height:22px;border-radius:6px;border:2px solid #E5E3DE;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0}
...
export default function App() {
  ...
  const [projTab, setProjTab] = useState("events");
  const [sheet, setSheet] = useState(null);
  const [sheetData, setSheetData] = useState({});

  // All form states at top level — no useState inside conditionals
  const [fProject, setFProject] = useState({ name: "", client: "", budget: "", color: PROJECT_COLORS[0] });
  const [fEvent, setFEvent] = useState({ date: todayStr(), amount: "", note: "" });
  const [fCost, setFCost] = useState({ name: "", executor: "", price: "", expenses: "", works: "", progress: 0 });
  const [fParticipant, setFParticipant] = useState({ contractorId: "", role: "", agentFee: "" });
  const [fContractor, setFContractor] = useState({ name: "", type: "Подрядчик", phone: "", note: "" });
  const [fPricelist, setFPricelist] = useState({ name: "" });
  const [fPlItem, setFPlItem] = useState({ name: "", unit: "м²", price: "" });
  const [fAgent, setFAgent] = useState({ name: "", amount: "", date: todayStr(), note: "" });
  const [fPayment, setFPayment] = useState({ date: todayStr(), amount: "", note: "" });
  ...
  const addCostItem = (ci) => updProj(projId, p => ({ ...p, costItems: [...p.costItems, { ...ci, id: uid(), payments: [], progress: ci.progress || 0, done: false }] }));
  ...
  const estTotal = () => { let t = 0; pricelists.forEach(pl => pl.items.forEach(it => { const k = `${pl.id}-${it.id}`; if (estSelections[k]) t += it.price * (estQty[k] || 1); })); return t; };
  const estLines = () => { const lines = []; pricelists.forEach(pl => pl.items.forEach(it => { const k = `${pl.id}-${it.id}`; if (estSelections[k]) lines.push({ name: it.name, unit: it.unit, qty: estQty[k] || 1, price: it.price }); })); return lines; };
  const printEstimate = () => {
    if (!proj) return;
    ...
    const html = `<html><head><meta charset="utf-8"><title>Смета</title><style>body{font-family:Arial,sans-serif;padding:32px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px 12px;text-align:left}th{background:#f5f5f5}tfoot td{font-weight:700}</style></head><body><h2>Смета</h2><p>Объект: ${proj.name} · Заказчик: ${proj.client}</p><table><thead><tr><th>Наименование</th><th>Ед.</th><th>Кол-во</th><th>Цена</th><th>Сумма</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="4">Итого</td><td>${estTotal().toLocaleString("ru-RU")} ₽</td></tr></tfoot></table></body></html>`;
    const w = window.open("", "_blank");
    if (w) { w.document.write(html); w.document.close(); setTimeout(() => w.print(), 400); }
  };

  const openSheet = (name, data = {}) => { setSheet(name); setSheetData(data); };
        ...
        <div className="screen">
          ...
          {mainTab === "home" && (<>
            ...
            <div className="sumcard">
              ...
              <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: -1, color: (totalPaid - totalExp) >= 0 ? "#5BC47A" : "#E8643A" }}>{fmtShort(totalPaid - totalExp)}</div>
              <div style={{ display: "flex", marginTop: 14 }}>
                {[{ l: "Получено", v: fmtShort(totalPaid), c: "#5BC47A" }, { l: "Расходы", v: fmtShort(totalExp), c: "#E8643A" }, { l: "Объектов", v: String(projects.length), c: "#888" }].map((s, i) => (
                  <div key={s.l} style={{ flex: 1, borderLeft: i > 0 ? "1px solid #2a2a2a" : "none", paddingLeft: i > 0 ? 14 : 0 }}>
                    <div style={{ fontSize: 10, color: "#666", textTransform: "uppercase", letterSpacing: .5 }}>{s.l}</div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: s.c, marginTop: 2 }}>{s.v}</div>
                  </div>
                ))}
              </div>
            </div>
            <div className="sec">
              <span className="sectl">Проекты</span>
              <span className="secact" onClick={() => { setFProject({ name: "", client: "", budget: "", color: PROJECT_COLORS[0] }); openSheet("add-project"); }}>+ Новый</span>
            </div>
            {projects.map(p => {
